...
def Estrategia_02(df, umbral_compra, take_profit, stop_loss, horas):
    ...
    for i, row in df.iterrows():
        ...
        if (row['var'] >= umbral_compra) & (hora in horas):
            if estado == 0:
                valor_compra = row['price']
...
# df_procesado.to_excel('data/precios_procesados_SQL.xlsx')
horas = [11, 12, 13]
# df_estrategia, df_desempeños, desempeño_separado = Estrategia_02(df_procesado, 3, 4, -1.5, horas)
desempeño_separado = Ejecutar_estrategias(df_procesado, Estrategia_02, 3, 4, -1.5, horas)
# df_estrategia.to_excel('data/df_estrategia.xlsx')
print(round(desempeño_separado*100, 1))
exit()
'''
...
stop_loss = -1.5
horas = [11, 12, 13]
desempeño_separado = Ejecutar_estrategias(df_procesado, Estrategia_02, umbral_compra, take_profit, stop_loss, horas)
